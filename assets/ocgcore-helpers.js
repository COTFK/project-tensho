...
export function duelNewCard(duel, team, duelist, code, controller, location, sequence, position) {
  ...
  const info_ptr = moduleInstance._malloc(32); // Safe size for struct
  
  // Set fields (all u8 and u32)
...
export function loadScript(duel, scriptContent, scriptName) {
  ...
  const content_ptr = moduleInstance._malloc(scriptContent.length);
  const name_ptr = moduleInstance._malloc(scriptName.length + 1);
  
  // Copy strings
  new Uint8Array(moduleInstance.wasmMemory.buffer, content_ptr, scriptContent.length).set(new TextEncoder().encode(scriptContent));
  new Uint8Array(moduleInstance.wasmMemory.buffer, name_ptr, scriptName.length + 1).set(new TextEncoder().encode(scriptName + '\0'));
  
  const result = moduleInstance._OCG_LoadScript(duel, content_ptr, scriptContent.length, name_ptr);
